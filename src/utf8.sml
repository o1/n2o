        ...
        let open Word
            ...
            fun decode (byte, (n, i, cp, a)) =
                ...
                in case i of
                        ...
                        else (0, 0, 0wx0, f (replacement, a))

                      | 1 => if w andb 0wxc0 = 0wx80 then
                        let val cp = (cp << 0w6) orb (w xorb 0wx80)
                        in if cp < overlong n then (0, 0, 0wx0, f (replacement, a))
                        else if cp > limit    then (0, 0, 0wx0, f (replacement, a))
                        else (0, 0, 0wx0, f (cp, a)) end
                        else decode (byte, (0, 0, 0wx0, f (replacement, a)))

                      | i => if w andb 0wxc0 = 0wx80 then
                        let val cp = (cp << 0w6) orb (w xorb 0wx80)
                        in (n, Int.-(i, 1), cp, a)
                        end else decode (byte, (0, 0, 0wx0, f (replacement, a)))
                end
        in
...
structure Utf8Encoder :> sig
    ...
    val codepointsToUtf8 :
        ((word * char list -> char list) -> char list -> 'a -> char list) -> 'a -> string
